...
def generate_mol_graphs(config: dict, 
                        id_batch: list, 
                        pdbbind_dir: str, 
    ...
    mol_graph_dir, mol_graph_ft = data_utils.get_output_paths(mol_graph_dir, data_dir, config['mol_graph_file_template'])
    ip_dir, ip_ft = data_utils.get_output_paths(ip_dir, data_dir, config['interaction_profile_file_template'])
    ...
    for t_idx, pdb_id in enumerate(id_batch):
        ...
        for record in ip:
            ...
            pdb_data_distances = np.array([mol_gen_utils.get_distance(x, interaction_xyz) for x in heavy_atom_pos])
        ...
        try:
            ...
            g = mol_gen_utils.generate_mol_graph(mol, torch.tensor(mol_y).float(), ATOM_LABELS)
