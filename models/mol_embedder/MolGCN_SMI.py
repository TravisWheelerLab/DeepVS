...
from torch_geometric.nn import GCN2Conv, LayerNorm, global_mean_pool, InstanceNorm, GraphNorm, GATv2Conv, BatchNorm
...
class MolGCN_SMI(torch.nn.Module):
    def __init__(self, feature_dim, hidden_dim, out_dim, **kwargs):
        ...
        self.conv1 = GATv2Conv(hidden_dim, hidden_dim, edge_dim=8, heads=4, concat=False)
        self.conv2 = GATv2Conv(hidden_dim, hidden_dim, edge_dim=8, heads=4, concat=False)
        self.conv3 = GATv2Conv(hidden_dim, hidden_dim, edge_dim=8, heads=4, concat=False)
        self.conv4 = GATv2Conv(hidden_dim, hidden_dim, edge_dim=8, heads=4, concat=False)
    ...
    def forward(self, data):
        ...
        x = self.dropout(x)
        h = self.act(x + self.conv1(x, edge_index, edge_attr))
        h = self.norm1(h)

        h = self.dropout(h)
        h = self.act(h + self.conv2(h, edge_index, edge_attr))
        h = self.norm2(h)

        h = self.dropout(h)
        h = self.act(h + self.conv3(h, edge_index, edge_attr))
        h = self.norm3(h)

        h = self.dropout(h)
        h = self.act(h + self.conv4(h, edge_index, edge_attr))
        h = self.norm4(h)
